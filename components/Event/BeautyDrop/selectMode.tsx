...
import { Heart, Home, Star, Target } from "lucide-react";
import {
  Area,
  Block,
  Box,
  ...
  return (
    <Area className="w-full max-w-4xl mx-auto">
      <Yard className="bg-white/95 backdrop-blur-sm border border-purple-100 shadow-lg rounded-xl p-6">
        {restrictedMessage ? (
          <Section className="flex flex-col items-center justify-center h-[80vh] bg-gradient-to-b from-[#F3E5F5] to-[#FFF3E0] rounded-xl p-6">
            <Block className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
              <Heart className="w-12 h-12 text-purple-400 mx-auto mb-4" />
              <RText className="text-2xl font-bold text-purple-700">
                Cannot Play!
              </RText>
              <RText className="text-base text-slate-600 mt-2">
                {restrictedMessage}
              </RText>
              <RText className="text-sm text-slate-500 mt-4">
                Thank you for your enthusiasm! Come back tomorrow for another
                round!
              </RText>
              <Group className="mt-6">
                <Button
                  onClick={onBackToMenu}
                  className="w-full bg-purple-500 text-white py-3 px-6 rounded-full text-base font-semibold hover:bg-purple-600 transition-colors"
                  label="Back to Menu"
                  icon={<Home className="w-5 h-5 mr-2" />}
                />
                <Button
                  onClick={() => onStartGame("practice")}
                  label={`Play Practice Mode`}
                  icon={<Star className="w-5 h-5 mr-2" />}
                  className="mt-2 w-full font-semibold py-3 rounded-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white"
                />
              </Group>
            </Block>
          </Section>
