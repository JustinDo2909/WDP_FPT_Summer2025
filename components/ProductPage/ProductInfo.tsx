...
export function ProductInfo({ productData }: { productData: IProduct }) {
  const { productCategory, productBrand } = productData;
  return (
    <>
      ...
      <Row className="flex items-center justify-start gap-0">
        <Wrap className="flex-start flex">
          ...
          <RText className="w-auto">{productData.rating}</RText>
      ...
      <Row className="flex items-center"><RText className="text-right bg-pink-100 rounded-full px-2">{productCategory.title}</RText></Row>
