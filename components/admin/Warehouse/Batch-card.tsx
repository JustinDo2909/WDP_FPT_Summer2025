...
export function BatchCard({ batch, product, onClick }: BatchCardProps) {
  ...
  return (
    ...
    >
      {/* Header */}
      <Area className="flex items-start justify-between mb-3">
        <Yard className="flex items-center space-x-3">
          {product && (
            <img
          ...
          <Yard>
            <RText className="font-semibold text-gray-900">#{batch.id}</RText>
        ...
        <Yard className="flex flex-col items-end space-y-1">
          <span
      ...
      <Area className="space-y-2">
        <Area className="flex items-center text-sm text-gray-600">
          ...
          <span>Received: {formatDate(batch.created_at)}</span>
        </Area>
        ...
        >
          ...
          <span>Expires: {formatDate(batch.expired_at)}</span>
          {isExpiringSoon() && (
      ...
      {/* Product Info */}
      {product && (
        <Area className="mt-3 pt-3 border-t border-gray-100">
          <Area className="flex items-center justify-between text-xs text-gray-500">
            <span>{product.productBrand?.title}</span>
